...
### Direct vs. persistent effects 

```{r, fig.width=12, fig.height=10}
# Plot variation in dARR between direct and persistent effect sizes
...
# Plot variation in precision between direct and persistent effect sizes
...
design_dat<-d.tree %>% group_by(tip.label) %>% summarise( 
                                               direct=brought_common_temp=="no", 
                                               persistent=brought_common_temp=="yes",
                                               CTmax=metric=="CTmax",
                                               LT50=metric=="LT50") # Summarise, for each species, whether they tested direct effects, persistent effects, CTmax, or LT50

design_dat<-distinct(design_dat) # Only keep unique rows
design_dat$direct=as.numeric(design_dat$direct) # convert TRUE/FALSE to binary values
...
design_dat<-design_dat %>% group_by(tip.label) %>% summarise(direct=sum(direct),        
...
design_dat$direct[design_dat$direct=="2"] <- "1" 
design_dat$persistent[design_dat$persistent=="2"] <- "1"
design_dat$CTmax[design_dat$CTmax=="2"] <- "1"
                                                               
design_dat$direct=as.factor(design_dat$direct) # convert back to factor for the plot
...
design_dat <- mutate(design_dat, 
                     persistence = 
                          ifelse(direct=="1"&persistent=="1", "both",
                          ifelse(direct=="1"&persistent=="0", "direct", "persistent"))) # If both direct and persistent, indicate "both", if only direct, indicate "direct", otherwise indicate "persistent"
...
p3<- p2 + new_scale_fill()+ geom_fruit(geom=geom_tile, mapping=aes(fill=persistence),offset=0.1,width=0.07, col="gray30")+scale_fill_manual(values=c("#7570B3","#D95F02","#1B9E77")) # Create tiles to indicate whether direct or persistent effects were assessed
...
# Number of studies, species, and effect sizes for direct and persistent designs
    `Studies (Direct)` = n_distinct(study_ID[brought_common_temp=="no"]),
    `Studies (Persistent)` = n_distinct(study_ID[brought_common_temp=="yes"]),
    
    `Species (Direct)` = n_distinct(genus_species[brought_common_temp=="no"]),
    `Species (Persistent)` = n_distinct(genus_species[brought_common_temp=="yes"]),   
    
    `Effect sizes (Direct)` = n_distinct(es_ID[brought_common_temp=="no"]),  
...
Here the effect sizes estimating the "direct" and "persistent" effects of developmental temperatures on heat tolerance are compared.
...
save(mod.habitat,  # Habitat without intercept
     ...
     mod.persist_cont, # Contrast between direct and persistent effects
     ...
     mod.persist_het_cont, # Contrasts between direct and persistent effects, with heteroscedasticity modeled
...
## *Analyses with direct designs only* {.tabset .tabset_fade .tabset_pills}
...
```{r, fig.height=10, fig.width=12}
d.direct<-filter(d,brought_common_temp=="no")
mod.direct_habitat<-run.model(d.direct, ~habitat-1)
summary(mod.direct_habitat)
mod_results(mod.direct_habitat, mod="habitat",data=d.direct, group="species_ID") 


my.orchard(mod.direct_habitat, xlab="dARR", mod="habitat", alpha=0.1, data=d.direct, group="species_ID", whisker=0.07)
```

#### Heteroscedasticity check 

```{r, fig.height=6, fig.width=12}
qplot(y = sqrt(residuals(mod.direct_habitat)^2), x = fitted(mod.direct_habitat)) + 
      geom_point() +
      geom_smooth(method = "lm") + 
      geom_hline(yintercept = 0, colour="red") 
```
...
```{r}
VCV_direct<-make_VCV_matrix(d.direct, V="Var_dARR", cluster="shared_trt_ID", obs="es_ID")
mod.direct_habitat_het <-  rma.mv(yi=dARR, 
                           V=VCV_direct, 
                           mods= ~habitat-1,
                           method="REML",
                           test="t",
                           ...
                           data=d.direct,
                           sparse=TRUE)
AICc(mod.direct_habitat)
AICc(mod.direct_habitat_het) 

summary(mod.direct_habitat_het)
mod_results(mod.direct_habitat_het, mod="habitat",data=d.direct, group="species_ID") 
r2_ml(mod.direct_habitat_het) 

my.orchard(mod.direct_habitat_het, mod="habitat", xlab="dARR", alpha=0.1, data=d.direct, group="species_ID", whisker=0.07) 
```


#### Mean direct effects with habitat weighted equally

```{r, fig.width=12, fig.height=8}
mod.adjusted_direct<-mod_results(model=mod.direct_habitat_het, data=d.direct, mod="1", weights="equal", group="species_ID")
mod.adjusted_direct$mod_table

my.orchard(mod.adjusted_direct, mod="1", xlab="dARR", alpha=0.1, data=d.direct, group="species_ID", whisker=0.04)
```
...
```{r, fig.height=10, fig.width=12}
mod.direct_taxa<-run.model(d.direct, ~taxonomic_group-1)
summary(mod.direct_taxa)
mod_results(mod.direct_taxa, mod="taxonomic_group",data=d.direct, group="species_ID") 

my.orchard(mod.direct_taxa, xlab="dARR", mod="taxonomic_group", data=d.direct, alpha=0.1, group="species_ID", whisker=0.09)
```

#### Heteroscedasticity check 

```{r, fig.height=6, fig.width=12}
qplot(y = sqrt(residuals(mod.direct_taxa)^2), x = fitted(mod.direct_taxa)) + 
      geom_point() +
      geom_smooth(method = "lm") + 
      geom_hline(yintercept = 0, colour="red") 
```
...
```{r, fig.height=10, fig.width=12}
mod.direct_taxa_het <-  rma.mv(yi=dARR, 
                           V=VCV_direct, 
                           mods= ~taxonomic_group-1,
                           method="REML",
                           test="t",
                           ...
                           data=d.direct,
                           sparse=TRUE)
AICc(mod.direct_taxa)
AICc(mod.direct_taxa_het) 

summary(mod.direct_taxa_het)
mod_results(mod.direct_taxa_het, mod="taxonomic_group",data=d.direct, group="species_ID") 
r2_ml(mod.direct_taxa_het) 

my.orchard(mod.direct_taxa_het, mod="taxonomic_group", xlab="dARR", alpha=0.1, data=d.direct, group="species_ID", whisker=0.09) 
```
...
#### Individual coefficients 
```{r, fig.height=10, fig.width=12}
mod.direct_metric<-run.model(d.direct, ~metric-1)
summary(mod.direct_metric)
mod_results(mod.direct_metric, mod="metric",data=d.direct, group="species_ID") 

my.orchard(mod.direct_metric, xlab="dARR", mod="metric", alpha=0.1, data=d.direct, group="species_ID", whisker=0.07)
```

#### Heteroscedasticity check 

```{r, fig.height=6, fig.width=12}
qplot(y = sqrt(residuals(mod.direct_metric)^2), x = fitted(mod.direct_metric)) + 
      geom_point() +
      geom_smooth(method = "lm") + 
      geom_hline(yintercept = 0, colour="red") 
```
...
```{r, fig.height=10, fig.width=12}
mod.direct_metric_het <-  rma.mv(yi=dARR, 
                           V=VCV_direct, 
                           mods= ~metric-1,
                           method="REML",
                           test="t",
                           ...
                           data=d.direct,
                           sparse=TRUE)
AICc(mod.direct_metric)
AICc(mod.direct_metric_het) 

summary(mod.direct_metric_het)
mod_results(mod.direct_metric_het, mod="metric",data=d.direct, group="species_ID") 
r2_ml(mod.direct_metric_het) 

my.orchard(mod.direct_metric_het, mod="metric", xlab="dARR", alpha=0.1, data=d.direct, group="species_ID", whisker=0.07) 
```
...
```{r, fig.height=10, fig.width=12}
mod.direct_ramping<-run.model(d.direct, ~scale(ramping, center=T, scale=F))
summary(mod.direct_ramping)

d.direct_ramping<-filter(d, ramping!="NA"&brought_common_temp=="no")
plot_continuous(d.direct_ramping, mod.direct_ramping, d.direct_ramping$ramping, "Heating rate (degrees/min)")
```

```{r, eval=F, include=F}
save(mod.direct_habitat, 
     mod.direct_habitat_het,
     mod.adjusted_direct,
     mod.direct_taxa, 
     mod.direct_taxa_het,
     mod.direct_metric, 
     mod.direct_metric_het,
     mod.direct_ramping, 
     
     file=here("RData","Sens_direct_only.RData"))

load(here("Rdata", "Sens_direct_only.RData")) 
```
...
### Direct effects
```{r}
d.mass.direct<-filter(d.mass, brought_common_temp=="no")
mod.body_mass.direct <- run.model(d.mass.direct, ~ log(body_mass))
summary(mod.body_mass.direct) # 411 effect sizes

plot_continuous(d.mass.direct, mod.body_mass.direct, log(d.mass.direct$body_mass), "ln body mass (g)")
```
...
plot_continuous(d.maturity, mod.maturity, log(d.maturity.noNA$age_maturity), "ln age at sexual maturity (years)")
```

### Direct effects
```{r}
d.maturity.direct<-filter(d.maturity, brought_common_temp=="no")
mod.maturity.direct<-run.model(d.maturity.direct, ~log(age_maturity)) 
summary(mod.maturity.direct) # 752 effect sizes

plot_continuous(d.maturity.direct, mod.maturity.direct, log(d.maturity.direct$age_maturity), "ln age at sexual maturity (years)")
```
...
```{r}
d_direct<-data.frame(category=c("A", "B", "C"),
                  count=c(700, 20, 146))
d_direct$fraction = d_direct$count / sum(d_direct$count)
d_direct$ymax = cumsum(d_direct$fraction)
d_direct$ymin = c(0, head(d_direct$ymax, n=-1))


ggplot(d_direct, aes(ymax=ymax, ymin=ymin, xmax=4, xmin=2.5, fill=category)) +
     geom_rect(col="black") +
...
ggsave("output/direct_effect_sizes.png", height=12, width=12, dpi=800, bg="transparent")
...
p3<- p2 + new_scale_fill()+ geom_fruit(geom=geom_tile, mapping=aes(fill=direct_persistent),offset=0.1,width=0.07, col="black", size=0.3)+scale_fill_manual(values=c("#9994EA","#EAD295","#95EA99"), name="Design")+theme(legend.direction = "vertical")
